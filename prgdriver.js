...
class prgdriver {
    ...
    async getBuffer() {
        ...
        return { protocol: Uint8Array.from([(packetSize == arduino.MAX_BYTES_PER_REQUEST ? 66 /* arduino.Command.BUF_NORMAL */ : 98 /* arduino.Command.BUF_END */), packetSize]), payload: this.progBytes.slice(currentPos, currentPos + packetSize) };
    } //getBuffer
    ...
    async getDirectoryLine() {
        ...
        return { protocol: Uint8Array.from([108 /* arduino.Command.DIR_END */, this.progName.length + 2]), payload: Uint8Array.from([blockLow, blockHigh].concat(Array.from(new TextEncoder().encode(this.progName.toUpperCase())))) };
    } //getDirectoryLine
