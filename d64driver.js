...
class d64driver {
    ...
    async getBuffer() {
        ...
        if (this.linkTrack != 0) {
            ...
            return { protocol: Uint8Array.from([66 /* arduino.Command.BUF_NORMAL */, arduino.MAX_BYTES_PER_REQUEST]), payload: this.progBytes.slice(currentPos, currentPos + arduino.MAX_BYTES_PER_REQUEST) };
        }
        else {
            ...
            return { protocol: Uint8Array.from([98 /* arduino.Command.BUF_END */, this.linkSector - 1]), payload: this.progBytes.slice(currentPos, currentPos + this.linkSector - 1) };
        }
    } //getBuffer
