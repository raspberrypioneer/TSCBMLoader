...
- A web browser which supports the Web Serial API is required. Chrome, Edge and Opera running on a Windows 11 PC were used for testing
...
## Build
- This project has been developed using TypeScript.
- The TypeScript Web Serial API types were initially installed with `npm install --save @types/w3c-web-serial`.
- Edit the TypeScript code in the [src folder](./src/) and compile into JavaScript using `tsc` within a terminal session.
