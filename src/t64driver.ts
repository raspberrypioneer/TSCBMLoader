...
class t64driver {
    ...
    subProgPos: number;  //Sub-program position
    dirPos: number;  //Directory position
    dirEntryTotal: number;  //Total number of directory entries
    dirListing: Array<Array<number>>;  //Directory listing
    progBytes: Uint8Array;  //Program byte array
    subProgBytes: Uint8Array;  //Sub-program byte array
    ...
    constructor(file: File) {
        ...
        this.subProgBytes = new Uint8Array(0);
    ...
    async getBuffer() {
        ...
        const packetSize = Math.min((this.subProgSize - currentPos), arduino.MAX_BYTES_PER_REQUEST);
        this.subProgPos += packetSize;
        return {protocol: Uint8Array.from([(packetSize == arduino.MAX_BYTES_PER_REQUEST ? arduino.Command.BUF_NORMAL : arduino.Command.BUF_END), packetSize]), payload: this.subProgBytes.slice(currentPos, currentPos + packetSize)};
    ...
    async openProgram(bytesProgName: number[]) {
    ...
    async getDirectoryLine() {
        ...
        if (this.dirPos == 0) {  //Disk header is reverse-video in quotes
            prefix = [arduino.REVERSE_CHAR, arduino.QUOTE_CHAR];
            suffix = [arduino.QUOTE_CHAR];
        }
        else {  //Normal entries are aligned after the block size, within quotes and end with the program type
            prefix = new Array(5-String(fileBlocks).length).fill(arduino.SPACE_CHAR).concat([arduino.QUOTE_CHAR]);
        ...
        if (this.dirPos >= this.dirListing.length-1) {  //Last directory entry
            this.dirPos = 0;
            return {protocol: Uint8Array.from([arduino.Command.DIR_END, line.length]), payload: line};
        }
        else {
            this.dirPos += 1;
            return {protocol: Uint8Array.from([arduino.Command.DIR_NORMAL, line.length]), payload: line};
        }

    }  //getDirectoryLine

    //Private method: Remove non breaking space character
    #replaceNonBreakSpace(element: number) {
