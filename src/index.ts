...
let imgConnect: HTMLImageElement;
let divReceivedMsg: HTMLDivElement;
let divDirectory: HTMLDivElement;
let divProgress: HTMLDivElement;
...
function setup() {
	...
	imgConnect = <HTMLImageElement>document.getElementById('imgConnect')!;
	divReceivedMsg = <HTMLDivElement>document.getElementById('divReceivedMsg')!;
	divDirectory = <HTMLDivElement>document.getElementById('divDirectory')!;
	divProgress = <HTMLDivElement>document.getElementById('divProgress')!;

	const selProgList = <HTMLSelectElement>document.getElementById('selProgList')!;
	const imgArt = <HTMLImageElement>document.getElementById('imgArt')!;
	const divProgressBar = <HTMLDivElement>document.getElementById('divProgressBar')!;
	const divTitle = <HTMLDivElement>document.getElementById('divTitle')!;
	const divInfo = <HTMLDivElement>document.getElementById('divInfo')!;
	const imgFileSelect = <HTMLImageElement>document.getElementById('imgFileSelect')!;
	const imgLogo = <HTMLImageElement>document.getElementById('imgLogo')!;
	const imgWeb = <HTMLImageElement>document.getElementById('imgWeb')!;
	...
	fileInput.oninput = function() {
		...
		if (selProgList.length == 1) {
			if (objLoader) {
				objLoader.setDriverForFile(fileInput.files![0] as File, testMode)
				.catch((err: Error) => {
	...
	selProgList.onchange = function() {
		if (objLoader && fileInput.files) {
			objLoader.setDriverForFile(fileInput.files![selProgList.selectedIndex] as File, testMode)
			.catch((err: Error) => {
				console.error(`Error reading file:`, err);
			});
			setImageArt(selectedRetro);
	...
	function setImageArt(selectedRetro: string) {
		...
		divInfo.innerHTML = `<p>${info[selProgList.value as keyof typeof info] || ""}</p>`;
	}
...
function receiveData(event: Event) {
