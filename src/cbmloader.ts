...
class jsCBMLoader {
	baud: number; // Baud rate for serial port
	port!: SerialPort; // Serial port object
	reader!: ReadableStreamBYOBReader; // Reader for the serial port
										...
										if (msg.length > 0) {
											...
											if ([0,2,8].includes(channel)) {
												...
												if (msg[0] == 36) {  //$ directory character
													...
													await this.sendArduino(Uint8Array.from([arduino.Command.DIR,0]));
												}
												else {
													...
													if (await this.driver.openProgram(msg)) {
														...
														await this.sendArduino(Uint8Array.from([arduino.Command.OPEN_FNF,1]));
												...
												if (channel == 1 || (channel == 15 && cmdName.substring(0, 2) == "S:")) {
													...
													await this.sendArduino(Uint8Array.from([arduino.Command.WRITE_FULL]));  //Acknowlege write instruction
												}
												else {
													if (cmdName == "M-R\xc4\xe5\x04" || cmdName == "M-R\xc6\xe5\x04") {
														//M-R for the drive number, see dos1541 rom $25c6 ($e5c4 - $c000 + 2)
														await this.sendArduino(Uint8Array.from([arduino.Command.BUF_END,2,52,177]));  //Bytes from $25c6. Used in Bruce Lee 2
													}
													else {
	...
	async sendArduino(data: Uint8Array) {
	...
	async sendArduinoMsg(data: ArduinoMsg) {
